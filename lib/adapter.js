...
module.exports = (function () {
    ...
    var adapter = {
        ...
        query: function(collectionName, statement, data, cb) {
            ...
            conn.connect(marshalConfig(dbs[collectionName].config), function(err) {
        ...
        create: function(collectionName, data, cb) {
            ...
            var conn = new mssql.Connection(marshalConfig(dbs[collectionName].config, data.dmKey));
        ...
        find: function(collectionName, options, cb) {
            ...
            if (options.where && options.where.dmKey) {
                options.dmKey = options.where.dmKey;
                delete(options.where["dmKey"]);
            ...
            var conn = new mssql.Connection(marshalConfig(dbs[collectionName].config, options.dmKey));
        ...
        update: function(collectionName, options, values, cb) {
            ...
            var conn = new mssql.Connection(marshalConfig(dbs[collectionName].config, values.dmKey));
    ...
    function marshalConfig(config, dmKey) {
        return {
            user: config.user,
            password: config.password,
            server: "10.0.0." + dmKey.a,
            port: config.port || 1433,
            database: dmKey.b,
            timeout: config.timeout || 5000,
            pool: {
                max: (config.pool && config.pool.max) ? config.pool.max : 1,
