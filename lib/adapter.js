...
module.exports = (function () {
    ...
    var adapter = {
        ...
        find: function(collectionName, options, cb) {
            ...
            //PRIME-6471, updated by lily on 5/13/2026
            var arch = undefined;
            if (collectionName=="pz_promotion"  && options.where){
                if ((options.where.start && options.where.end) || options.where.id) {
                    arch = {
                        start : options.where.start,
                        end : options.where.end,
                        id : options.where.id
                    };
                    delete  options.where.start;
                    delete  options.where.end;
                }
                else {
                    arch = {all:true};
                }
            ...
            if (arch) {
              ...
              statement = statement.replace(/\*/g,'code, name, inactive, rules, actions, distondist, rebate, rebateentityid,manualexecution, images, stores, storeids, transform, props, id, createdAt, updatedAt, ordinal, description,pubstatus, rebatetaxable, canceltime, 0 as archived') 
              + ' UNION ALL ' 
              +  sql.selectQuery("dw.pz_promotion", options).replace(/\*/g, 'code, name, inactive, rules, actions, distondist, rebate, rebateentityid,manualexecution, images, stores, storeids, transform, props, id, createdAt, updatedAt, ordinal, description,pubstatus, rebatetaxable, canceltime, 1 as archived');
        ...
        destroy: function(collectionName, options, cb) {
            ...
            conn.connect(function __DELETE__(err) {
                ...
                var request = new mssql.Request(conn);
                //PRIME-6361, updated by lily on 3/18/2026
                if (tableName == "hq.pz_user"||tableName == "hq.reports_schedule" || tableName == "acc.misc_map") 
                    delete options.where.accountid;
