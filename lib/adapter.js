...
module.exports = (function () {
    ...
    var adapter = {
        ...
        query: function(collectionName, statement, data, cb) {
            ...
            if (_.isFunction(data)) {
                console.log(collectionName, statement, data)
        ...
        updateAddress: function(conn, values, cb){
            ...
            request.input('address', mssql.NVarChar(4001), JSON.stringify(address__address));
            request.execute('pz.sp_update_address', function(err, recordsets, returnValue) {
                if (err && err.message.indexOf('JSON text is not properly formatted')==0)
                    cb(null, [])
                else
                    cb(err,recordsets[0]);
            });
        ...
        create: function(collectionName, data, cb) {
            ...
            conn.connect(function __CREATE__(err, recordset) {
                ...
                function __create(address){
                    ...
                    var statement = sql.insertQuery(translateTableName(dbs[collectionName].identity), data, dbs[collectionName].definition);
                    var request = new mssql.Request(conn);
        ...
        find: function(collectionName, options, cb) {
            ...
            if (options.where && options.where.__user) {
                ...
                if (collectionName.substr(0,3)!='hq.')
        ...
        update: function(collectionName, options, values, cb) {
            ...
            conn.connect(function __UPDATE__(err) {
                ...
                if (tableName.indexOf('.')<0) tableName = "pz." + translateTableName(tableName);;
                ...
                function __update(address) {
                    ...
                    var request = new mssql.Request(conn);
                    var statement ='UPDATE ' + tableName + ' SET ' + sql.updateCriteria(dbs[collectionName].identity, values, dbs[collectionName].definition) + ' output inserted.* ';
                    statement += sql.serializeOptions(dbs[collectionName].identity, options);
                    request.query(statement, function(err, recordset) {
                        if (err) return cb(err);
                        if (!!address)recordset[0].address__address = address;
                        cb(null, recordset[0]);
                    });
                }
    ...
    function marshalConfig(config,user) {
        var server = config.hq ? '10.0.0.99':user.host||"10.0.0.121";
        var port = config.hq ? 31433:config.port || 1433;
        var database = config.hq?'s2k': user.dbname||"kwstest"
        return {
            user: config.user,
