...
utils.buildSelectStatement = function(criteria, table) {
  ...
  if (criteria.address__address)
    query += "*, (select * from " + (table=='hq.pz_account'?'hq':'pz') + ".pz_address where id in (select convert(int, value) from string_split(a.address,',')) for json path) as address__address "
  else
    query += '* '
  delete criteria.address__address;
