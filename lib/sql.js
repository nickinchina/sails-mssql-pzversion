...
var skipKey = function(collectionName, key){
	switch (collectionName){
		...
		case 'acc.misc_map':
			return false;
		case 'hq.pz_store':
			return key=="accountid" || key == 'siteid';
        default:
...
var sql = {
	...
	insertQuery: function (collectionName, data, definition,isTriggerBased) {
		var tableName = collectionName;
		if (tableName.indexOf('.')<0) tableName = "pz." + tableName;
		//return 'INSERT INTO ' + tableName + ' ' + '(' + sql.attributes(collectionName, data) + ')  VALUES (' + 
		if (isTriggerBased(tableName)) {
			if (tableName.toLowerCase()=='pz.pz_taxons')
				return 'declare @t table(id uniqueidentifier); INSERT INTO ' + tableName + ' (' + sql.attributes(collectionName, data) + ')  output inserted.id into @t VALUES (' +
	...
	prepareValue: function(collectionName, value, attrName, definition) {
		if (definition && definition.type && definition.type.toLowerCase()=='json'){
			if (!!value)
				return  "N'" + JSON.stringify(_.isString(value)?JSON.parse(value):value).replace(/'/g, "''") +  "'" ;
			return "NULL";
		}
